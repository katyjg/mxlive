...
@register.inclusion_tag('filter.html')
def filter(ol, spec):
...
@register.inclusion_tag('lists/object_details.html', takes_context=True)
def object_details(context, obj):
...
def object_fields(obj):
    ...
    for field_name in obj._meta.admin.list_display:
        row_class = ''
        try:
