...
Remote Connections in MxLIVE are tailored to provide a list of allowed users to a remote service through an API. Scripts
for accessing the API for NoMachine Servers and SSH Servers are provided. A history of remote connections can be viewed
using the "Connections" icon in the toolbar.

Users who should be granted access to a remote server are added to the list of a specific remote connection, which they
can then connect to through the NoMachine Client or SSH, depending on the configuration of the server.
...
NoMachine Server
^^^^^^^^^^^^^^^^
A python script `deploy/nxupdate.py` is set to run on each NoMachine Server periodically to fetch the new user list from
MxLIVE. A history of connection times and statuses as reported by `nxserver --history` is sent back to MxLIVE.

SSH Server
^^^^^^^^^^
A python script `deploy/sshupdate.py` is set to run on each SSH server periodically to fetch the new user list from
MxLIVE. A history of connection times and statuses as reported by `last -Rw --time-format iso` is sent back to MxLIVE.
