...
for section, subsection in _URL_META.items():
    for key, params in subsection.items():
        ...
        if params.get('delete', False):
            _dynamic_patterns.append(
                url(r'%s/(?P<id>\d+)/delete/$' % base_url,
        ...
        if params.get('staff_comments', True):
            _dynamic_patterns.append(
                url(r'%s/(?P<id>\d+)/staff_comments/add/$' % (base_url),
...
# Special Staff Cases
urlpatterns = [
    url(r'^$', staff_home, {}, 'staff-home'),
    url(r'^shipments/$', ShipmentList.as_view(), name='shipment-list' ),
    url(r'^profile/edit/(?P<id>\d+)/$', edit_profile,
     {'form': ProjectForm, 'template': 'objforms/form_base.html'}, 'staff-profile-edit'),
    url(r'^users/new/$', create_project, {'form': NewUserForm, 'template': 'objforms/form_base.html'},
     'staff-create-profile'),

    # Dewars
    url(r'^shipping/dewar/receive/(?P<id>\d+)$', receive_shipment,
    ...
    # Runlists
    url(r'^runlist/(?P<runlist_id>\d+)/container/basic/(?P<obj_id>\d+)/$', container_basic_object_list,
        {'model': Container, 'template': 'staff/lists/basic_container_list.html'},
        'staff-container-basic-list'),
    url(r'^runlist/(?P<runlist_id>\d+)/adaptor/basic/$', adaptor_basic_object_list,
    ...
    url(r'^runlist/(?P<dest_id>\d+)/widget/.*/container/(?P<obj_id>\d+)/loc/(?P<loc_id>\w{1,2})/$',
     add_existing_object, {'destination': Runlist, 'obj': Container},
     'staff-runlist-add-container'),
    url(r'^runlist/(?P<id>\d+)/load/$', staff_action_object,
     {'model': Runlist, 'form': RunlistEmptyForm, 'template': 'objforms/form_base.html',
      'action': 'load'}, 'staff-runlist-load'),
    url(r'^runlist/(?P<id>\d+)/unload/$', staff_action_object,
     {'model': Runlist, 'form': RunlistEmptyForm, 'template': 'objforms/form_base.html',
      'action': 'unload'}, 'staff-runlist-complete'),
    url(r'^runlist/(?P<id>\d+)/history/$', object_history, {'model': Runlist},
...
urlpatterns += [
    ...
    # Shipments
    url(r'^shipping/shipment/(?P<id>\d+)/return/$', action_object, {
            'model': Shipment, 'form': ShipmentReturnForm, 'template': 'objforms/form_base.html',
            'action': 'return'}, 'staff-shipment-return'),
    url(r'^shipping/shipment/(?P<id>\d+)/label/$', shipment_pdf, {
            'model': Shipment, 'format': 'return_label'
        }, 'staff-shipment-label'),
    url(r'^shipping/project/(?P<id>\d+)/form/$', shipment_pdf, {
            'model': Project, 'format': 'return_label'
        }, 'staff-shipment-form'),
    url(r'^shipping/shipment/(?P<id>\d+)/protocol/$', shipment_pdf, {
            'model': Shipment, 'format': 'protocol'
        }, 'staff-shipment-protocol'),
    url(r'^shipping/shipment/(?P<id>\d+)/progress/$', object_detail, {
            'model': Shipment, 'template': 'users/entries/progress_report.html'
        }, 'lims-shipment-progress'),

    # Runlists
    url(r'^runlist/(?P<src_id>\d+)/container/(?P<obj_id>\d+)/remove/$', remove_object,
     {'source': Runlist, 'obj': Container}, 'staff-runlist-remove-container'),
    url(r'^runlist/(?P<id>\d+)/protocol/$', shipment_pdf, {'model': Runlist, 'format': 'runlist'},
     'staff-runlist-pdf'),
    url(r'^runlist/(?P<id>\d+)/staff_comments/add/$', staff_comments,
     {'model': Runlist, 'form': RunlistCommentsForm, }, 'staff-comments-runlist-add'),
]

urlpatterns += [
    url(r'^experiment/$', django.shortcuts.redirect, {'url': '/staff/experiment/request/'}),
    url(r'^samples/$', django.shortcuts.redirect, {'url': '/staff/samples/crystal/'}),
]

urlpatterns += [
    url(r'^link/media/(?P<path>.*)$', django.views.static.serve,
