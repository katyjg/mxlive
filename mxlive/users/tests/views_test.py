...
from mxlive.users.views import create_and_update_project_and_laboratory
from mxlive.users.views import project_required
from mxlive.users.views import manager_required
from mxlive.users.views import project_assurance
from mxlive.users.views import home
from mxlive.users.views import show_project
from mxlive.users.views import shipment_pdf
from mxlive.users.views import shipment_xls
from mxlive.users.views import upload_shipment
from mxlive.users.views import edit_object_inline
from mxlive.users.views import object_list
from mxlive.users.views import create_object
from mxlive.users.views import change_priority
from mxlive.users.views import add_existing_object

from mxlive.users.forms import ShipmentUploadForm
from mxlive.users.forms import ShipmentForm
...
from mxlive.users.tests.test_utils import UserMock
from mxlive.users.tests.test_utils import RequestMock
from mxlive.users.tests.test_utils import convert_dict_of_QuerySet_to_dict_of_list
...
class ShowProjectTest(DjangoTestCase):
    ...
    def test_empty(self):
        ...
        self.assertEqual('users/project.html', response.rendered_args[0])
...
class CreateObject_Shipment_Test(DjangoTestCase):
    ...
    def test_POST_valid_with_redirect(self):
        ...
        request.POST['label'] = 'label'
        self.assertEqual(1, Shipment.objects.count())
        response = create_object(request, Shipment, ShipmentForm, redirect='users-shipment-list')
        self.assertEqual(2, Shipment.objects.count())
...
#       
#        self.assertEqual(('users/redirect.html', {'redirect': '/users/shipping/shipment/'}), response.rendered_args)
