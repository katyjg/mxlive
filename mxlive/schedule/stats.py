...
def beamtime_stats(objlist, filters):
    period = 'start__year' in filters and 'month' or 'year'
    ...
    return {
        'details': [
            ...
            downtime_stats(cancelled, downtime_filters, field, period, periods, period_names)
        ]
    }

def beamtime_summary(beamline, period='year', **filters):
    periods = get_beamline_periods(period=period, beamline=beamline)
    ...
    if PUBLICATIONS:
        # Average H-index
        active_users_info = beamtime_info.exclude(project__isnull=True).values(field, 'project__username')
        active_users = {}
        for au in active_users_info:
            active_users.setdefault(au[field], []).append(au['project__username'])
        hindex = h_indices(Project.objects.filter(pk__in=beamtime_info.values_list('project', flat=True).distinct()))
        hindex_data = []
        for i, per in enumerate(periods):
            hindex_data.append({
                period.title(): period_names[i],
...
def downtime_stats(objlist, filters, field, period, periods, period_names):
    # Downtime
    downtime_info = Downtime.objects.with_duration().filter(**filters)
