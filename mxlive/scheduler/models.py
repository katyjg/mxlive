...
class Proposal(models.Model):
    ...
    '''
    EXPIRY = enum.Enum(_('Submitted'), _('Active'), _('Expired'))
    ...
    expiry = models.CharField(_('Expiry'), max_length=1, choices=EXPIRY.get_choices())
    ...
    def save(self, *args, **kwargs):
        ...
        if self.expiration is None:
            self.expiry = self.EXPIRY.SUBMITTED
        elif self.expiration >= today:
            self.expiry = self.EXPIRY.ACTIVE
        else:  # self.expiration < today
            self.expiry = self.EXPIRY.EXPIRED    
